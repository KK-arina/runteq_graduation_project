...
# 【理由】ベースイメージを 3.4.10 にしたので、Gemfile の指定も合わせる。ここが 3.4.7 のままだと bundle install が不一致で失敗する。
ruby "3.4.10"

# ==================== フレームワーク ====================

# Rails: Webアプリケーションフレームワーク
# 【理由】既存の ~> 7.2.3 と同じ3セグメント表記に統一（8.1.x パッチは許容・8.2 系は自動で入れない保守的指定）
gem "rails", "~> 8.1.3"
...
# 【理由】Ruby 3.4 で csv が default gem から分離されたため、CSVエクスポートで
#   警告/将来のLoadErrorを防ぐべく明示追加（既存の ostruct 追加と同じ理由）。
gem "csv"
