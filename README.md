...
週次振り返り機能 - できなかった理由を明文化して記録
AI分析連携機能 - 外部AIに現状を共有し、「なぜ？」を3回繰り返して真の原因を究明
改善計画の自動反映機能 - AI提案を貼り付けるだけで、改善タスクが自動追加される
...
今週の結果を確認（各習慣の達成率）
振り返り入力（うまくいかなかったこと、表面的な理由、背景）
「AIに共有」ボタンで現状サマリーをコピー
外部のAIサービス（ChatGPT/Claude/Gemini等）に貼り付けて分析依頼
AIの回答（真の原因、改善タスク）をコピー
アプリに貼り付けて「一括反映」→ 自動的にタスク追加・目標更新
...
ユーザーの獲得について
MVP段階： 自分自身がヘビーユーザーとして3〜6ヶ月使い込み、習慣の継続率は上がるか、AI分析は役立つか、毎日続けられるかを検証します。
MVP後： 技術ブログ（Qiita/Zenn）での記事化、GitHub公開、SNSでの情報発信、友人への紹介を検討しますが、ユーザー獲得は二の次。まずは自分が満足できるものを作ることに集中します。
サービスの差別化ポイント・推しポイント
既存サービスとの比較
機能HabiticaTodoistNotionHabitFlow習慣トラッキング◯△◯◯タスク管理◯◯◯◯進捗率自動計算◯△手動◯PDCA振り返り✗✗◯※◯AI原因分析✗✗✗◯改善計画自動生成✗✗✗◯
...
ユーザー認証機能 - ユーザー登録・ログイン・ログアウト、パスワード暗号化（bcrypt）
習慣管理機能 - 習慣のCRUD、測定タイプ（毎日実施/冊数/時間）、日次記録、進捗率の自動計算
月次目標設定機能 - 月初に目標を設定、できない日の事前登録、達成状況の表示
タスク管理機能 - タスクのCRUD、優先度（Must/Should/Could）、タイプ（習慣関連/通常/改善タスク）
ダッシュボード - 今月の進捗サマリー、今日の習慣チェックリスト、今日のタスク一覧
週次振り返り機能 - 今週の結果表示、振り返り入力フォーム、保存機能
AI分析用プロンプト生成機能 - 「AIに共有」ボタンで最適化されたプロンプトをクリップボードにコピー
AI提案の自動反映機能 - AIの回答を貼り付けるテキストエリア、「一括反映」ボタン、パース処理
...
開発期間：12週間（約3ヶ月）
前提条件： Rails初心者（Runteqカリキュラム学習中）、週15〜20時間の開発時間確保、2026年3月までに完成
...
※HotwireとTailwind CSSはRunteqカリキュラムで明示的に扱われているか確認できていませんが、Rails 7にデフォルトで含まれており、学習コストが低い（2〜3日）ため採用。Week 1で学習期間を確保。
データベース

PostgreSQL 16.11

2025年11月リリース、2028年11月までサポート
PostgreSQL公式サイトで確認済み
実績が豊富、JSON型のサポート
...
実装予定の機能と技術
1. AI分析のパース処理 - Ruby標準ライブラリ（String#split、正規表現）
ruby# AIに統一フォーマットで出力させ、パイプ区切りをパース
...
2. 進捗率の自動計算 - Railsモデルメソッド、PostgreSQL集計関数、日付計算
rubyclass Goal < ApplicationRecord
...
3. 自動更新 - after_saveコールバック
rubyclass DailyRecord < ApplicationRecord
...
4. JSON型の活用 - PostgreSQLのJSON型で柔軟なデータ保存
ruby# goals テーブル - できない日のリスト
