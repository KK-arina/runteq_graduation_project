...
| Week I | 品質・テスト・デプロイ | #I-1〜#I-6 | 22 | 🟡 進行中 |
| **合計** | | **70** | **233** | |
...
│   │   ├── weekly_reflections_controller.rb # 変更（G-9）: confirm_proposals に habit_modify/habit_delete/task_modify/goal_review 処理追加・show に @current_purpose 追加
│   │   ├── tasks_controller.rb            # 変更: toggle_complete にモーダル再注入追加・destroy追加（#C-3/#C-4バグ修正）
│   │   ├── sessions_controller.rb         # 変更（E-4）: ログイン後ディープリンク遷移・determine_redirect_path 追加・オンボーディング優先設計
│   │   ├── users_controller.rb            # ユーザー登録
│   │   ├── errors_controller.rb           # カスタムエラーページ
│   │   ├── pages_controller.rb            # ランディングページ
│   │   ├── user_purposes_controller.rb    # 変更(#I-1): 危機記録の input_snapshot に5キー追加
...
│   │   └── analytics_controller.rb        # #H-4 新規: グラフ・進捗分析ページ（period フィルター・N+1対策済み集計・touch_analytics_viewed_at!）
│   ├── models/
...
│   │   ├── user_purpose.rb                # 変更（E-1）: 5フィールドを presence: true 必須化・include CrisisDetector と crisis_text_fields を維持
...
│   │   ├── ai_user_profile.rb                # H-8 新規: AIユーザープロファイルモデル（stale? メソッド・UNIQUE制約）
│   │   ├── password_reset_token.rb        # F-4 新規: トークン管理（generate_token_for / find_by_raw_token / valid_token? / expire!・BCryptハッシュ化・includes(:user)でN+1対応）
│   │   └── concerns/
...
│   │   ├── weekly_reflection_analysis_job.rb # 変更（G-9）: PROMPT_VERSION v2.0・VALID_ACTION_TYPES クラスレベル定数・maxOutputTokens 8192・goal_review 固定キー形式・build_prompt に current_habits/current_tasks 引数追加
│   │   ├── purpose_analysis_job.rb        # 変更（G-9）: broadcast_state_update 重複定義（533行目付近）を削除し1本化・dashboard 向けに completed 時は空HTML を配信
...
│   │   └── update_ai_profile_job.rb          # H-8 新規: AIプロファイル更新ジョブ（単一ユーザー / 全ユーザー週次一括更新）
│   ├── mailers/
...
│   │   ├── password_mailer.rb             # F-4 新規: パスワードリセットメイラー（reset_password・deliver_later）
│   │   ├── weekly_report_mailer.rb        # G-2 新規: 週次レポートメイラー（helper :application 明示・new_weekly_reflection_url で deep_link_url 生成）
...
│       ├── weekly_reflections/            # 変更（E-1）: new.html.erb に気分スコアUI・必須バッジ・インラインエラー追加、show.html.erb に★表示追加
...
│       │   ├── _ai_proposal_modal.html.erb        # #E-3 新規: 14-A AI提案プレビューモーダル（デスクトップ中央モーダル/スマホボトムシート・ai-proposal-modal Stimulusコントローラー接続）
│       │   ├── _ai_proposal_modal_content.html.erb # #E-3 新規: 14-A モーダルコンテンツ（AI免責バナー・PMVVとの整合性・変更サマリー・除外ボタン・確定ボタン）
│       │   ├── _ai_proposal_banner.html.erb        # #E-3 新規: AI提案バナー（Turbo Stream target id="weekly_reflection_ai_banner"・分析中/完了状態切替）
│       │   └── show.html.erb              # 変更（G-9）: AI免責バナー直後に「✨ 来週のAI提案を確認する →」ボタンを追加・_ai_proposal_modal パーシャルを render・CustomEvent でモーダル直接起動・@current_purpose アサイン追加
...
│       │   ├── _bottom_navigation.html.erb    # H-1 新規: スマホ専用Bottom Navigation（5タブ・バッジ・SafeArea・aria属性）
│       │   └── _empty_state.html.erb          # H-7 新規: 全画面共通Empty Stateパーシャル（icon/title/description/cta_label/cta_path/cta_class/testid）
│       ├── task_mailer/                                # #C-5 新規: アラーム通知メールビュー
│       │   ├── alarm_notification.html.erb             # HTMLメール本文
│       │   └── alarm_notification.text.erb             # テキストメール本文
│       ├── weekly_report_mailer/                           # G-2 新規: 週次レポートメールビュー
│       │   ├── report.html.erb                             # インラインスタイル・3色カラー・CTA ボタン
│       │   └── report.text.erb                             # HTML 非対応クライアント向けプレーンテキスト版
│       ├── tasks/
│       │   ├── index.html.erb             # 9番: タスク一覧
│       │   ├── new.html.erb               # 10番: タスク新規作成（H-6変更: text-base統一・py-3タップ領域・優先度カードh-full高さ統一）
...
│       │   ├── _voice_field.html.erb      # 変更（E-1）: 必須バッジ・エラー時赤枠・インラインエラー追加（new/edit/step5 に自動反映）
│       │   └── _analysis_status_banner.html.erb  # 変更（D-11）: failed バナー大幅改善（エラー種別→日本語変換・タイムアウト時「AIなしで続行する」ボタン・補足テキスト常時表示）
...
│       ├── onboardings/
│       │   ├── step2.html.erb             # H-5 新規・H-6 変更: 1/2 習慣テンプレート選択ページ（H-6: input に text-base 追加・iOS自動ズーム防止）
...
│       ├── csv_export_mailer/                 # G-5 新規: CSV生成完了メールビュー
│       │   ├── ready.html.erb
│       │   └── ready.text.erb
│       ├── settings/
...
│       │   └── edit.html.erb              # F-4 新規: 26番 新パスワード入力フォーム
│       ├── errors/
│       │   ├── token_invalid.html.erb     # F-4 新規: 29番 トークン無効エラーページ（HTTP 404）
│       │   └── too_many_requests.html.erb     # #F-5: 429カスタムエラーページ
│       ├── password_mailer/
│       │   ├── reset_password.html.erb    # F-4 新規: HTMLメールテンプレート（インラインCSS）
│       │   └── reset_password.text.erb    # F-4 新規: テキストメールテンプレート
│       ├── terms_agreement/
│       │   └── show.html.erb              # F-3 新規: OAuth初回ログイン用同意ページ
│       ├── analytics/
│       │   └── index.html.erb             # #H-4 新規: グラフ・進捗分析ページ本体（Empty State・月次サマリー・2種グラフ・ストリークテーブル）
│       └── pages/
...
│   │   ├── YYYYMMDDHHMMSS_add_deleted_at_to_users.rb  # F-6: deleted_at 追加・email/provider+uid の部分インデックス変更（up/down 分離）
│   │   ├── YYYYMMDDHHMMSS_add_last_analytics_viewed_at_to_user_settings.rb  # #H-4: バッジリセット用カラム
│   │   ├── YYYYMMDDHHMMSS_add_pmvv_banner_dismissed_at_to_user_settings.rb  # #H-9: PMVV完了バナーを✖で閉じた日時（リロード後も残す判定用）
│   │   └── YYYYMMDDHHMMSS_add_reflection_banner_dismissed_at_to_user_settings.rb  # #H-9: 振り返り完了バナーを✖で閉じた日時（PMVVと対称）
...
│   │   └── rack_attack.rb                 # #F-5: throttle 4種・safelist（localhost/Docker IP）・429レスポンス設定（`unless Rails.env.test?` で全体を囲む）
│   ├── locales/
│   │   ├── en.yml                         # 英語ロケール
│   │   └── ja.yml                         # 変更: Task属性名・エラーメッセージ追加（#C-1）・UserSetting ai_analysis_monthly_limit エラーメッセージ追加（#D-4）
│   └── environments/
│       ├── development.rb                 # 変更: letter_opener設定追加（#A-4）・CSS preload警告抑制追加（E-5: config.action_view.preload_links_header = false・respond_to?でRailsバージョン差異吸収）
...
└── test/
    ...
    │   ├── csv_download_token_service_test.rb # G-5 新規
    │   └── user_context_builder_service_test.rb  # H-8 新規: 14 runs, 22 assertions（context_text_for / call / stale? / generate_context_summary / UpdateAiProfileJobエンキュー）
    ├── fixtures/
    │   └── weekly_reflection_task_summaries.yml  # #C-4 新規
    ├── mailers/
    │   ├── weekly_report_mailer_test.rb   # G-2 新規（3件：振り返りあり・なし・習慣なし）
    │   └── previews/
    │       └── weekly_report_mailer_preview.rb  # G-2 新規: http://localhost:3000/rails/mailers/weekly_report_mailer/report でブラウザ確認
    ├── jobs/
    │   ├── weekly_reflection_analysis_job_test.rb  # 変更（G-9）: G-9テスト5件追加（PROMPT_VERSION/build_prompt習慣一覧/タスク一覧/空配列/build_input_snapshot）
    │   ├── weekly_reflection_analysis_job_test.rb  # #D-4 新規（8件：正常系・nil返却・PMVV有無・月次上限・discard_on）
    │   ├── weekly_report_job_test.rb      # G-2 新規（4件：送信確認・無効ユーザー除外・退会ユーザー除外・堅牢性stub）
    │   ├── monthly_ai_count_reset_job_test.rb  # G-8 新規: 7テスト（月初リセット・月初以外スキップ・月末境界値・例外なし完了・cron設定確認・maximum確認・14-Bモーダル解消確認）
    │   └── csv_export_job_test.rb  # #I-1 新規: 非同期CSV→メール送信・email未設定スキップ
    ├── models/
    ...
    │   ├── task_test.rb                   # 変更: soft_delete・ai_generated? テスト4件追加（#C-3・計28件）・#I-1でscope/enum/コールバック追記
    │   ├── weekly_reflection_habit_summary_test.rb  # 変更（E-1）: create! に3フィールド追加
    │   ├── weekly_reflection_task_summary_test.rb   # 変更（E-1）: create! に3フィールド追加
    │   ├── ai_analysis_test.rb            # 変更（D-9）: D-9 テスト 11 ケース追加（正常系5・異常系7・weekly_reflection は create! で作成・private ヘルパーをファイル末尾に配置）
    │   ├── password_reset_token_test.rb           # F-4 新規（15 runs）
    │   ├── user_purpose_test.rb           # #I-1 新規: analysis_state enum・version検証・active_for/current_for
    │   ├── numeric_habit_achievement_test.rb  # #I-1 新規: 数値型達成率の境界値（floor / round2 の2経路）
    │   └── concerns/
    ...
    └── controllers/
        ...
        ├── tasks_controller_test.rb       # 変更（H-7）: タスクEmpty State（tasks-empty-state / tasks-done-empty-state）の表示テストを3件追加・#I-1で作成→完了→アーカイブ通しフロー追記
        ├── tasks_ai_edit_controller_test.rb  # #C-7 新規: ai_edit/ai_update（9件: 正常フロー・直接アクセス403・優先度変更不可・バリデーションエラー・他ユーザー404）
        ├── dashboards_controller_test.rb  # 変更（H-7）: ダッシュボード習慣0件のEmpty State表示・CTAリンク表示テストを2件追加
        ...
        ├── weekly_reflections_controller_test.rb  # 変更（G-9）: G-9テスト3件追加（task_modify優先度更新/存在しないタスクスキップ/goal_review→user_purpose_pathリダイレクト）
        ├── onboardings_controller_test.rb  # 変更（E-1）: valid_purpose_params ヘルパー追加・complete テストに5フィールド追加
        ├── sessions_controller_test.rb    # E-4 新規: ディープリンク正常系・外部URL拒否・ダブルスラッシュ拒否・javascript:拒否・空文字拒否（10ケース）
        ├── password_resets_controller_test.rb # F-4 新規（18 runs）
        └── csv_exports_controller_test.rb  # G-5 新規（10件）
```
