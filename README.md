...
- ✅ 先週未完了振り返りの優先表示・PDCAロック解除フロー修正完了
- ✅ 振り返りフォームのHTTPメソッド（PATCH→POST）バグ修正完了
- ✅ weekly_reflections テーブルに user_id + week_start_date のユニーク制約追加完了
- ✅ downcase_email の nil 安全化（email.to_s.downcase）完了
- ✅ User#locked? コメント修正（火曜1:00でも monday? が true になるという誤記を削除）完了
- ✅ Issue #37〜#41 対応中（Week 6：3月31日レビュー依頼提出予定）
...
#### ✅ Issue #36: 最終デプロイ・本番確認

<br>

- `bin/docker-entrypoint` に条件付き seed 処理を追加（`SEED_IN_PRODUCTION=true` 時のみ `db:seed` を実行）
- `render.yaml` に `startCommand` を追加（デプロイのたびに `db:migrate` を自動実行）
- `exec bin/rails server` により Rails が PID 1 になり Graceful Shutdown を保証
- `seeds.rb` の本番ガードを環境変数フラグ方式（`SEED_IN_PRODUCTION`）に変更
- `SEED_IN_PRODUCTION=true` を明示指定した場合のみ本番実行を許可（二重ロック設計）
- 3秒待機メッセージで誤実行時の Ctrl+C による中断猶予を実装
- Render 環境変数に `SEED_IN_PRODUCTION=true` を一時追加 → デプロイ → 確認後削除の安全な運用フローを確立
- 本番 URL（https://habitflow-web.onrender.com）で全機能の動作確認完了
- 全テスト成功: 202 runs, 604 assertions, 0 failures, 0 errors, 0 skips

<br>

### Week 6（3/1〜3/31）: 本番環境最終動作確認・ドキュメント整備
...
- ユーザー管理・習慣管理・ダッシュボード・週次振り返り・PDCAロック・エラーハンドリングを網羅
- AM4:00ちょうどの境界値テストを追加（月曜AM4:00でロック発動することを検証）
- `docs/production_check_issue_37.md` を新規作成（本番環境手動確認チェックリスト9項目）
- **重大バグ修正**: `config.time_zone = "Tokyo"` / `config.active_record.default_timezone = :local` を追加（未設定のままだとRailsがUTCで動作し、本番環境でロック時刻が9時間ズレる致命的バグが発生する）
- **先週未完了振り返り優先表示**: `WeeklyReflectionsController` に `find_pending_last_week_reflection` を追加し、先週未完了の振り返りがあれば今週より優先して表示する仕様に変更
- **フォームPATCH→POSTバグ修正**: `new.html.erb` の `form_with` に `url: weekly_reflections_path, method: :post` を明示し、既存レコードを渡したときに PATCH が送信されて404になるバグを修正
- **DBユニーク制約追加**: `weekly_reflections` に `user_id + week_start_date` の unique index を追加（1ユーザー1週1振り返りをDBレベルで保証）
- **downcase_email nil安全化**: `email.downcase` → `email.to_s.downcase` に変更
- **locked? コメント修正**: 「火曜1:00でも monday? が true になる」という誤ったコメントを削除し正確な説明に修正
...
│   │   └── YYYYMMDDHHMMSS_add_unique_index_to_weekly_reflections.rb # user_id + week_start_date のユニーク制約追加（if_not_exists: true で冪等性保証）
│   ├── schema.rb                              # データベーススキーマ
│   └── seeds.rb                               # デモ用サンプルデータ（本番誤実行防止ガード・SEED_IN_PRODUCTION フラグ方式・3秒待機・デモユーザー・習慣6個・過去3週分の記録168件・週次振り返り3件・スナップショット12件）
├── docs/
...
│   │   └── production_final_check_test.rb # Issue #37 本番環境最終動作確認テスト（19テストケース・AM4:00境界値・PDCAロック・エラーハンドリング）／先週未完了振り返り優先表示・フォームPATCH→POST修正・ユニーク制約追加・downcase_email nil安全化を含む
│   ├── controllers/
...
├── render.yaml                           # Renderデプロイ設定（startCommand: db:migrate自動実行 + exec Graceful Shutdown対応）
└── Gemfile                               # Gem依存関係
```
<br>
...
## 最終デプロイ・本番確認（Issue #36）

<br>

### render.yaml の startCommand 設計
```yaml
...
| 要素 | 内容 | 理由 |
|------|------|------|
| `bin/rails db:migrate` | デプロイのたびに未実行マイグレーションを自動適用 | 手動実行忘れを防止・冪等性あり |
| `&&` | マイグレーション成功時のみサーバーを起動 | マイグレーション失敗状態で起動するのを防ぐ安全弁 |
| `exec` | シェルプロセスを Rails プロセスで置き換え | Rails が PID 1 になり SIGTERM を直接受信 → Graceful Shutdown |
| `-b 0.0.0.0` | 全 IP アドレスからの接続を受け付ける | Render のロードバランサーからのリクエストを受け取るため必須 |

<br>

**`exec` がなかった場合の問題**：<br>
exec なし → シェル（PID 1）→ Rails（PID 2）という親子関係になります。<br>
Render がデプロイ時に SIGTERM を送っても、PID 1 のシェルにしか届かず Rails には転送されません。<br>
その結果、リクエスト処理中でも強制終了（SIGKILL）されてしまいます。<br>
`exec` により Rails 自身が PID 1 になることで SIGTERM を直接受信し、正常終了処理が保証されます。<br>

<br>

### bin/docker-entrypoint の条件付き seed 設計

<br>

**Render 無料プランでは Shell 機能が使えない（有料プラン限定）** という制約から、<br>
`startCommand` の条件分岐では `&&` や `if` がシェルとして解釈されないことが判明しました。<br>
そのため seed 実行ロジックは `bin/docker-entrypoint` に実装しています。<br>
```bash
# bin/docker-entrypoint（本番環境の処理抜粋）
if [ "$RAILS_ENV" = "production" ]; then
  ./bin/rails db:prepare

  if [ "$SEED_IN_PRODUCTION" = "true" ]; then
    echo "SEED_IN_PRODUCTION=true detected. Running db:seed..."
    ./bin/rails db:seed
  fi
fi
...
1. Render ダッシュボードの Environment Variables に `SEED_IN_PRODUCTION=true` を一時追加<br>
2. デプロイ実行 → entrypoint が自動的に `db:seed` を実行<br>
3. 動作確認後、`SEED_IN_PRODUCTION` を Environment Variables から削除<br>
4. 次回デプロイ以降は seed が実行されない（全データ削除リスクをゼロに）<br>
...
**設計の意図**：<br>
講師レビュー用のデモデータを本番 DB に投入する必要がある一方、<br>
誤実行による全データ削除リスクを最小化するため、環境変数フラグを明示指定しない限り実行を拒否する二重ロック設計を採用しました。<br>
3秒待機メッセージは誤実行に気づいたときに Ctrl+C で中断できる最後の猶予です。<br>
...
### Issue #36 で得た教訓：本番デプロイのポイント

<br>

**① exec を必ず付ける**<br>
`startCommand` や `CMD` でサーバーを起動する際、`exec` を付けないと Rails が PID 1 にならず Graceful Shutdown が機能しません。<br>
デプロイのたびにリクエストが強制切断されるリスクがあります。<br>

<br>

**② db:migrate を startCommand に含める**<br>
マイグレーションを手動実行する運用はヒューマンエラーの温床です。<br>
`startCommand` に `bin/rails db:migrate &&` を含めることで、デプロイと同時に自動適用されます。<br>
`migrate` は冪等（何度実行しても安全）なので毎回実行しても問題ありません。<br>

<br>

**③ startCommand のシェル構文に注意する**<br>
`startCommand` に `&&` や `if` を書いても、環境によってはシェルとして解釈されない場合があります。<br>
複雑な条件分岐は `bin/docker-entrypoint` 等のシェルスクリプトに実装する方が確実です。<br>

<br>

**④ seeds.rb の本番ガードは環境変数フラグ方式で設計する**<br>
`Rails.env.production? → abort` だけでは「意図的な実行」ができません。<br>
`ENV["SEED_IN_PRODUCTION"].blank?` との組み合わせにより、<br>
「誤実行は防ぎつつ、必要なときだけ明示的に実行できる」安全な設計になります。<br>

<br>

**⑤ RAILS_MASTER_KEY は Render ダッシュボードで事前確認する**<br>
`RAILS_MASTER_KEY` が未設定だと `Missing secret_key_base` エラーでサーバーが起動しません。<br>
デプロイ前に Render ダッシュボードの Environment Variables で設定済みかを必ず確認してください。<br>
...
`config.time_zone` が未設定だったため Rails がUTCで動作しており、本番環境でロック時刻が9時間ズレる致命的バグが発覚しました。
```ruby
# config/application.rb
...
| 設定項目 | 理由 |
|---------|------|
| `config.time_zone = "Tokyo"` | `Time.current` が常にJST（UTC+9）で動作。AM4:00判定・週の区切りをJST基準に統一 |
| `config.active_record.default_timezone = :local` | DBへの日時読み書きにJSTを使用。タイムゾーン変換の複雑さを排除 |
...
- `WeeklyReflectionsController` の `new` / `create` に `find_pending_last_week_reflection` を追加
- 先週未完了の振り返りがある場合、今週分より優先してフォームに表示
- ロック中ユーザーが振り返りページを開くと自動的に先週分が表示される
```ruby
...
### 振り返りフォームのHTTPメソッドバグ修正

<br>

**原因**: `form_with model: @weekly_reflection` は `persisted? = true` のレコードを渡すと自動的に `PATCH /weekly_reflections/:id` を送信する。しかし routes に `update` アクションが存在しないため404になっていた。
```erb
<%# 修正後: url と method を明示して常に POST /weekly_reflections に送信 %>
...
`if_not_exists: true` を使うことでマイグレーションを冪等（安全に再実行可能）にしています。
...
| PDCAロック | 4 | ロック中作成拒否・AM3:59ロックなし・AM4:00ロックあり・振り返り完了でロック解除 |
