...
class User < ApplicationRecord
  ...
  # 既存の has_many :habits の下に追記する

  # --------------------------------------------------------------------------
  # has_many :weekly_reflections
  # 1人のUserは複数のWeeklyReflectionを持つという関係を定義する。
  # dependent: :destroy → ユーザー削除時に紐づく振り返りも全て削除する
  #   ※ DBのCASCADE設定と二重で保証することで、データ孤立を確実に防ぐ
  # --------------------------------------------------------------------------
  has_many :weekly_reflections, dependent: :destroy

  # ============================================================================
  # 【追加】1対多: 1人のユーザーは複数の習慣記録を持つ
  # ============================================================================
  # - dependent: :destroy → ユーザーが削除されたら、そのユーザーの習慣記録も全て削除
